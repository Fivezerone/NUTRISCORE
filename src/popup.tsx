...
import { listProducts, listScans, listScores } from './storage/nutriscore-db';
...
function PopupApp() {
  ...
  useEffect(() => {
    ...
    Promise.all([listProducts(), listScans(), listScores()])
      .then(([products, scans, scores]) => {
        if (!active) {
          return;
        }

        setState({ loading: false, products, scans, scores });
      })
      .catch(() => {
        if (!active) {
          return;
    ...
    <>
      ...
      <div class="shell">
        <section class="card">
          ...
          <div class="stats">
            <div class="stat"><strong>{state.products.length}</strong><span>Products</span></div>
            <div class="stat"><strong>{state.scans.length}</strong><span>Scans</span></div>
            <div class="stat"><strong>{state.scores.length}</strong><span>Scores</span></div>
          </div>
          ...
          <div class="list">
            {state.loading ? (
              <div class="item">
                ...
                <small>Reading products, scans, and scores from the frozen schema.</small>
              </div>
            ) : recentProduct ? (
              <div class="item">
                <strong>{recentProduct.name}</strong>
                <small>{recentProduct.priceText ?? 'No price captured yet'}</small>
                <small>{recentProduct.packSize ?? 'No pack size captured yet'}</small>
              </div>
            ) : (
              <div class="item">
                <strong>No stored products yet</strong>
                <small>Run the Jumia scraper to populate the local store.</small>
              </div>
            )}
          </div>
        </section>
